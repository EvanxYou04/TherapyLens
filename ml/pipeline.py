...
def run_pipeline(turns: list[dict]) -> dict:
	therapist_turns = [t for t in turns if str(t.get("speaker", "")).lower().startswith("therap")]
	client_turns = [t for t in turns if str(t.get("speaker", "")).lower().startswith("client")]
	...
	talk_ratio = (
		therapist_word_count / max(1, therapist_word_count + client_word_count)
	)
